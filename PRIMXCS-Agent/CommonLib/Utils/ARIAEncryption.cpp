#include "stdafx.h"
#include "ARIAEncryption.h"
#include "SHA512.h"
#include "Base64.h"
#include <bcrypt.h>
#pragma comment(lib, "Bcrypt.lib")


#define ARIA_BLOCK_BYTE_SIZE			16

#ifndef SHA512_DIGEST_LENGTH
	#define SHA512_DIGEST_LENGTH		64
#endif

const unsigned char SaltData[SHA512_DIGEST_LENGTH] = {	0x60, 0x81, 0x4f, 0xdc, 0x22, 0x2a, 0x90, 0x88, 0x46, 0xee, 0xb8, 0x14, 0xde, 0x5e, 0x0b, 0xdb,
												0xb7, 0xfd, 0x93, 0x26, 0x36, 0x3f, 0xf7, 0xcc, 0x34, 0xa5, 0xe5, 0xf1, 0x71, 0xd8, 0x31, 0x15,
												0xe7, 0xc8, 0x37, 0x6d, 0x8d, 0xd5, 0x4e, 0xa9, 0x6c, 0x56, 0xf4, 0xea, 0x65, 0x7a, 0xae, 0x08,
												0xba, 0x78, 0x25, 0x2e, 0x1c, 0xa6, 0xb4, 0xc6, 0xe8, 0xdd, 0x74, 0x1f, 0x4b, 0xbd, 0x8b, 0x8a };


const BYTE S[4][256] = {
	// S-box type 1
	{
		 },
		// S-box type 2
		{
			 },
			// inverse of S-box type 1
			{
				 },
				// inverse of S-box type 2
				{
					 }
};
// constants used in the round key generation
const BYTE KRK[3][16] = {
	{ 0x51, 0x7c, 0xc1, 0xb7, 0x27, 0x22, 0x0a, 0x94, 0xfe, 0x13, 0xab, 0xe8, 0xfa, 0x9a, 0x6e, 0xe0 },
	{ 0x6d, 0xb1, 0x4a, 0xcc, 0x9e, 0x21, 0xc8, 0x20, 0xff, 0x28, 0xb1, 0xd5, 0xef, 0x5d, 0xe2, 0xb0 },
	{ 0xdb, 0x92, 0x37, 0x1d, 0x21, 0x26, 0xe9, 0x70, 0x03, 0x24, 0x97, 0x75, 0x04, 0xe8, 0xc9, 0x0e }
};


CARIAEncryption::CARIAEncryption(ARIA_ENC_KEY_TYPE enKeyType)
{
	m_enKeyType = ARIA_ENC_KEY_256; // enKeyType;

	int nMasterKeyLen = GetMasterKeyByte(m_enKeyType);
	int nRoundKeyLen = GetRoundKeyByte(m_enKeyType);

	m_pMasterKey = new BYTE[nMasterKeyLen];
	m_pEncRoundKey = new BYTE[ARIA_BLOCK_BYTE_SIZE * (nRoundKeyLen + 1)];
	m_pDecRoundKey = new BYTE[ARIA_BLOCK_BYTE_SIZE * (nRoundKeyLen + 1)];
		
	BYTE TempKey[32] = { 0x0d, 0xd4, 0x5c, 0xa9, 0x1e, 0x65, 0xcd, 0x19, 0xfa, 0x8d, 0x12, 0xea, 0xa1, 0x66, 0x42, 0x51, 0xa8, 0x4e, 0x5c, 0x70, 0x60, 0x22, 0x00, 0x30, 0xaa, 0x77, 0x6f, 0xf8, 0x3f, 0x66, 0xe3, 0x06};
	
	memcpy(m_pMasterKey, TempKey, nMasterKeyLen);

}

CARIAEncryption::~CARIAEncryption()
{
	if (NULL != m_pMasterKey) {
		delete[] m_pMasterKey;
		m_pMasterKey = NULL;
	}
	if (NULL != m_pEncRoundKey) {
		delete[] m_pEncRoundKey;
		m_pEncRoundKey = NULL;
	}
	if (NULL != m_pDecRoundKey) {
		delete[] m_pDecRoundKey;
		m_pDecRoundKey = NULL;
	}
}

// Diffusion Layer
void CARIAEncryption::DL(const BYTE *i, BYTE *o)
{
	BYTE T;

	T = i[3] ^ i[4] ^ i[9] ^ i[14];
	o[0] = i[6] ^ i[8] ^ i[13] ^ T;
	o[5] = i[1] ^ i[10] ^ i[15] ^ T;
	o[11] = i[2] ^ i[7] ^ i[12] ^ T;
	o[14] = i[0] ^ i[5] ^ i[11] ^ T;
	T = i[2] ^ i[5] ^ i[8] ^ i[15];
	o[1] = i[7] ^ i[9] ^ i[12] ^ T;
	o[4] = i[0] ^ i[11] ^ i[14] ^ T;
	o[10] = i[3] ^ i[6] ^ i[13] ^ T;
	o[15] = i[1] ^ i[4] ^ i[10] ^ T;
	T = i[1] ^ i[6] ^ i[11] ^ i[12];
	o[2] = i[4] ^ i[10] ^ i[15] ^ T;
	o[7] = i[3] ^ i[8] ^ i[13] ^ T;
	o[9] = i[0] ^ i[5] ^ i[14] ^ T;
	o[12] = i[2] ^ i[7] ^ i[9] ^ T;
	T = i[0] ^ i[7] ^ i[10] ^ i[13];
	o[3] = i[5] ^ i[11] ^ i[14] ^ T;
	o[6] = i[2] ^ i[9] ^ i[12] ^ T;
	o[8] = i[1] ^ i[4] ^ i[15] ^ T;
	o[13] = i[3] ^ i[6] ^ i[8] ^ T;
}
// Right-rotate 128 bit source string s by n bits and XOR it to target string t
void CARIAEncryption::RotXOR(const BYTE *s, int n, BYTE *t)
{
	int i, q;

	q = n / 8; n %= 8;
	for (i = 0; i < 16; i++) {
		t[(q + i) % 16] ^= (s[i] >> n);
		if (n != 0) t[(q + i + 1) % 16] ^= (s[i] << (8 - n));
	}
}

// Encryption round key generation rountine
// w0 : master key, e : encryption round keys
int CARIAEncryption::EncKeySetup(const BYTE *w0, BYTE *e, int keyBits) {
	int  i, R = (keyBits + 256) / 32, q;
	BYTE t[16], w1[16], w2[16], w3[16];

	q = (keyBits - 128) / 64;
	for (i = 0; i < 16; i++) t[i] = S[i % 4][KRK[q][i] ^ w0[i]];
	DL(t, w1);
	if (R == 14)
		for (i = 0; i < 8; i++) w1[i] ^= w0[16 + i];
	else if (R == 16)
		for (i = 0; i < 16; i++) w1[i] ^= w0[16 + i];

	q = (q == 2) ? 0 : (q + 1);
	for (i = 0; i < 16; i++) t[i] = S[(2 + i) % 4][KRK[q][i] ^ w1[i]];
	DL(t, w2);
	for (i = 0; i < 16; i++) w2[i] ^= w0[i];

	q = (q == 2) ? 0 : (q + 1);
	for (i = 0; i < 16; i++) t[i] = S[i % 4][KRK[q][i] ^ w2[i]];
	DL(t, w3);
	for (i = 0; i < 16; i++) w3[i] ^= w1[i];

	for (i = 0; i < 16 * (R + 1); i++) e[i] = 0;

	RotXOR(w0, 0, e); RotXOR(w1, 19, e);
	RotXOR(w1, 0, e + 16); RotXOR(w2, 19, e + 16);
	RotXOR(w2, 0, e + 32); RotXOR(w3, 19, e + 32);
	RotXOR(w3, 0, e + 48); RotXOR(w0, 19, e + 48);
	RotXOR(w0, 0, e + 64); RotXOR(w1, 31, e + 64);
	RotXOR(w1, 0, e + 80); RotXOR(w2, 31, e + 80);
	RotXOR(w2, 0, e + 96); RotXOR(w3, 31, e + 96);
	RotXOR(w3, 0, e + 112); RotXOR(w0, 31, e + 112);
	RotXOR(w0, 0, e + 128); RotXOR(w1, 67, e + 128);
	RotXOR(w1, 0, e + 144); RotXOR(w2, 67, e + 144);
	RotXOR(w2, 0, e + 160); RotXOR(w3, 67, e + 160);
	RotXOR(w3, 0, e + 176); RotXOR(w0, 67, e + 176);
	RotXOR(w0, 0, e + 192); RotXOR(w1, 97, e + 192);
	if (R > 12) {
		RotXOR(w1, 0, e + 208); RotXOR(w2, 97, e + 208);
		RotXOR(w2, 0, e + 224); RotXOR(w3, 97, e + 224);
	}
	if (R > 14) {
		RotXOR(w3, 0, e + 240); RotXOR(w0, 97, e + 240);
		RotXOR(w0, 0, e + 256); RotXOR(w1, 109, e + 256);
	}
	return R;
}
// Decryption round key generation rountine
// w0 : maskter key, d : decryption round keys
int CARIAEncryption::DecKeySetup(const BYTE *w0, BYTE *d, int keyBits) {
	int  i, j, R;
	BYTE t[16];

	R = EncKeySetup(w0, d, keyBits);
	for (j = 0; j < 16; j++) {
		t[j] = d[j];
		d[j] = d[16 * R + j];
		d[16 * R + j] = t[j];
	}
	for (i = 1; i <= R / 2; i++) {
		DL(d + i * 16, t);
		DL(d + (R - i) * 16, d + i * 16);
		for (j = 0; j < 16; j++) d[(R - i) * 16 + j] = t[j];
	}
	return R;
}
// Encryption and decryption rountine
// p: plain text, e: round keys, c: ciphertext
void CARIAEncryption::Crypt(const BYTE *p, int R, const BYTE *e, BYTE *c)
{
	int i, j;
	BYTE t[16];

	for (j = 0; j < 16; j++) c[j] = p[j];
	for (i = 0; i < R / 2; i++)
	{
		for (j = 0; j < 16; j++) t[j] = S[j % 4][e[j] ^ c[j]];
		DL(t, c); e += 16;
		for (j = 0; j < 16; j++) t[j] = S[(2 + j) % 4][e[j] ^ c[j]];
		DL(t, c); e += 16;
	}
	DL(c, t);
	for (j = 0; j < 16; j++) c[j] = e[j] ^ t[j];
}

int CARIAEncryption::GetMasterKeyByte(ARIA_ENC_KEY_TYPE enKeyType)
{
	if (128 == enKeyType) return (128 / 8);
	if (192 == enKeyType) return (192 / 8);
	if (256 == enKeyType) return (256 / 8);
	return 0;
}

int CARIAEncryption::GetRoundKeyByte(ARIA_ENC_KEY_TYPE enKeyType)
{
	if (128 == enKeyType) return 12;
	if (192 == enKeyType) return 14;
	if (256 == enKeyType) return 16;
	return 0;
}

int CARIAEncryption::EncryptBlock(BYTE*& pOutputData, BYTE* pInputData, unsigned int nInputDataLen)
{
	m_nEncRounds = EncKeySetup(m_pMasterKey, m_pEncRoundKey, (int)m_enKeyType);

	int nLoopCount = (nInputDataLen / ARIA_BLOCK_BYTE_SIZE);
	int nAddCount = (0 < (nInputDataLen % ARIA_BLOCK_BYTE_SIZE)? 1 : 0);

	int nOutputDataLen = ARIA_BLOCK_BYTE_SIZE * (nLoopCount + nAddCount);
	pOutputData = new BYTE[nOutputDataLen];
	ZeroMemory(pOutputData, nOutputDataLen);

	BYTE* pSeekInput = pInputData;
	BYTE* pSeekOutput = pOutputData;

	for (int i = 0; i < nLoopCount; i++) {
		Crypt(pSeekInput, m_nEncRounds, m_pEncRoundKey, pSeekOutput);
		pSeekInput = pSeekInput + ARIA_BLOCK_BYTE_SIZE;
		pSeekOutput = pSeekOutput + ARIA_BLOCK_BYTE_SIZE;
	}
	if (1 == nAddCount) {
		BYTE* TempData = new BYTE[ARIA_BLOCK_BYTE_SIZE];
		ZeroMemory(TempData, ARIA_BLOCK_BYTE_SIZE);
		memcpy(TempData, pSeekInput, (nInputDataLen % ARIA_BLOCK_BYTE_SIZE));
		Crypt(TempData, m_nEncRounds, m_pEncRoundKey, pSeekOutput);
		delete[] TempData;
	}
	return nOutputDataLen;
}

int CARIAEncryption::DecryptBlock(BYTE*& pOutputData, BYTE* pInputData, unsigned int nInputDataLen)
{
	m_nDecRounds = DecKeySetup(m_pMasterKey, m_pDecRoundKey, (int)m_enKeyType);

	int nLoopCount = (nInputDataLen / ARIA_BLOCK_BYTE_SIZE);
	int nAddCount = (0 < (nInputDataLen % ARIA_BLOCK_BYTE_SIZE) ? 1 : 0);
	if (1 <= nAddCount) {
		return 0;
	}

	int nOutputDataLen = ARIA_BLOCK_BYTE_SIZE * nLoopCount;
	pOutputData = new BYTE[nOutputDataLen];
	ZeroMemory(pOutputData, nOutputDataLen);

	BYTE* pSeekInput = pInputData;
	BYTE* pSeekOutput = pOutputData;

	for (int i = 0; i < nLoopCount; i++) {
		Crypt(pSeekInput, m_nDecRounds, m_pDecRoundKey, pSeekOutput);
		pSeekInput = pSeekInput + ARIA_BLOCK_BYTE_SIZE;
		pSeekOutput = pSeekOutput + ARIA_BLOCK_BYTE_SIZE;
	}
	return nOutputDataLen;
}

CString CARIAEncryption::GetSaltSHA512(CString strValue)
{
	CString strEncryptData;
	if (0 == strValue.GetLength()) {
		return strEncryptData;
	}

	// 암호화할 문자열을 저장할 메모리 할당
	int nPlainTextLen = strValue.GetLength() * sizeof(TCHAR);
	unsigned char* pPlainText = new unsigned char[nPlainTextLen +1];
	ZeroMemory(pPlainText, nPlainTextLen +1);
	memcpy(pPlainText, (LPCTSTR)strValue, nPlainTextLen);

	// 암호화할 문자열 SHA512 해시 생성
	unsigned char Digest[SHA512_DIGEST_LENGTH] = { 0 };

	CSHA512 SHAChecksum512;
	SHAChecksum512.init();

	SHAChecksum512.update(SaltData, SHA512_DIGEST_LENGTH);
	SHAChecksum512.update(pPlainText, nPlainTextLen);
	SHAChecksum512.final(Digest);

	CBase64 Base64;
	int Base64Len = Base64.EncodeGetRequiredLength(SHA512_DIGEST_LENGTH);
	char* Base64md = new char[Base64Len + 1];
	ZeroMemory(Base64md, Base64Len + 1);
	if (TRUE == Base64.Encode((unsigned char*)Digest, SHA512_DIGEST_LENGTH, Base64md, &Base64Len)) {
		strEncryptData = AtoWConverter(Base64md);
	}
	_SafeDeleteArray(Base64md);
	_SafeDeleteArray(pPlainText);

	return strEncryptData;
}

CString CARIAEncryption::EncryptString(CString strValue)
{
	CString strEncryptData;
	if (0 == strValue.GetLength()) {
		return strEncryptData;
	}

	// 암호화할 문자열을 저장할 메모리 할당
	UINT nEncryptDataLen = strValue.GetLength();
	TCHAR* pData = new TCHAR[nEncryptDataLen+1];
	wmemset(pData, 0x00, nEncryptDataLen+1);
	_tcsncpy_s(pData, nEncryptDataLen+1, strValue, _TRUNCATE);

	BYTE* pOutputData = NULL;	
	int nOutputDataLen = EncryptBlock(pOutputData,(BYTE*)pData, (nEncryptDataLen+1)* sizeof(TCHAR));
	
	CBase64 Base64;
	int Base64Len = Base64.EncodeGetRequiredLength(nOutputDataLen);
	char* Base64md = new char[Base64Len + 1];
	ZeroMemory(Base64md, Base64Len + 1);
	if (TRUE == Base64.Encode((unsigned char*)pOutputData, nOutputDataLen, Base64md, &Base64Len)) {
		strEncryptData = AtoWConverter(Base64md);
	}

	_SafeDeleteArray(Base64md);
	_SafeDeleteArray(pData);
	_SafeDeleteArray(pOutputData);

	return strEncryptData;
}

CString CARIAEncryption::DecryptString(CString strValue)
{
	CBase64 Base64Encoder;
	int nEncryptLen = strValue.GetLength();
	int nBase64Len = Base64Encoder.DecodeGetRequiredLength(nEncryptLen);
	unsigned char* pBase64md = new unsigned char[nBase64Len];
	ZeroMemory(pBase64md, nBase64Len);

	char* pData = new char[nEncryptLen+2];
	strncpy_s((char*)pData, nEncryptLen+2, WtoAConverter(strValue), _TRUNCATE);
	
	if (TRUE == Base64Encoder.Decode(pData, nEncryptLen, pBase64md, &nBase64Len)) {
		unsigned char* pDecryptData = NULL;
		int nDecryptLen = DecryptBlock(pDecryptData, pBase64md, nBase64Len);
		strValue = (TCHAR*)pDecryptData;
		_SafeDeleteArray(pDecryptData);
	}
	_SafeDeleteArray(pBase64md);
	_SafeDeleteArray(pData);

	return strValue;
}

unsigned short CARIAEncryption::CheckSum(unsigned short *buffer, int size)
{
	unsigned long cksum = 0;
	while (size >1)
	{
		cksum += *buffer++;
		size -= sizeof(unsigned short);
	}
	if (size)
		cksum += *(unsigned short*)buffer;

	cksum = (cksum >> 16) + (cksum & 0xffff);
	cksum += (cksum >> 16);
	return (unsigned short)(~cksum);
}

CString CARIAEncryption::SetBaseKeyCreateData()
{
	BYTE  RandomData[202] = { 0 };

	NTSTATUS hr = BCryptGenRandom(
		NULL,
		(BYTE*)RandomData,
		200,
		BCRYPT_USE_SYSTEM_PREFERRED_RNG
	);

	if (FAILED(hr)) {
		for (int i = 0; i < 200; i++) {
			srand((unsigned int)GetTickCount());
			RandomData[i] = _Random(1, 254);
		}
	}

	unsigned short nCheckSum = CheckSum((unsigned short*)RandomData, 100);

	memcpy(RandomData + 200, &nCheckSum, sizeof(unsigned short));

	BYTE TempData[100] = { 0 };
	for (int i = 0; i < 100; i++) {
		TempData[i] = RandomData[i] ^ RandomData[i + 100];
	}

	BYTE HashDataH[SHA512_DIGEST_LENGTH] = { 0 };
	BYTE HashDataL[SHA512_DIGEST_LENGTH] = { 0 };

	CSHA512 SHAChecksum512;
	SHAChecksum512.init();
	SHAChecksum512.update(RandomData, 50);
	SHAChecksum512.final(HashDataH);

	SHAChecksum512.init();
	SHAChecksum512.update((RandomData + 50), 50);
	SHAChecksum512.final(HashDataL);

	BYTE KeyData[ARIA_MASTER_KEY_LEN] = { 0 };
	for (int i = 0; i < ARIA_MASTER_KEY_LEN; i++) {
		KeyData[i] = HashDataH[i] ^ HashDataL[i+30];
	}

	memcpy(m_pMasterKey, KeyData, 32);
	
	CString strBaseKeyData;
	CBase64 Base64Encoder;
	int nBase64Len = Base64Encoder.EncodeGetRequiredLength(202);
	char* pBase64md = new char[nBase64Len + 1];
	ZeroMemory(pBase64md, nBase64Len + 1);
	if (TRUE == Base64Encoder.Encode((unsigned char*)RandomData, 202, pBase64md, &nBase64Len, BASE64_FLAG_NOCRLF)) {
		strBaseKeyData = AtoWConverter(pBase64md);
	}
	delete[] pBase64md;
	strBaseKeyData.Trim();

	return strBaseKeyData;
}

BOOL CARIAEncryption::SetBaseKeyData(CString strMasterKey)
{
	BOOL bRe = FALSE;

	CBase64 Base64Encoder;
	int nEncryptLen = strMasterKey.GetLength();
	int nBase64Len = Base64Encoder.DecodeGetRequiredLength(nEncryptLen);
	unsigned char* RandomData = new unsigned char[nBase64Len];
	ZeroMemory(RandomData, nBase64Len);

	char* pData = new char[nEncryptLen + 2];
	strncpy_s((char*)pData, nEncryptLen + 2, WtoAConverter(strMasterKey), _TRUNCATE);

	if (TRUE == Base64Encoder.Decode(pData, nEncryptLen, RandomData, &nBase64Len)) {

		if (202 == nBase64Len) {
			unsigned short nCheckSum = CheckSum((unsigned short*)RandomData, 100);
			unsigned short nCheckSum2 = 0;

			memcpy(&nCheckSum2, RandomData+200, sizeof(unsigned short));

			if (nCheckSum2 == nCheckSum) {
				int a = 0;
			}
			
			BYTE TempData[100] = { 0 };
			for (int i = 0; i < 100; i++) {
				TempData[i] = RandomData[i] ^ RandomData[i + 100];
			}

			BYTE HashDataH[SHA512_DIGEST_LENGTH] = { 0 };
			BYTE HashDataL[SHA512_DIGEST_LENGTH] = { 0 };

			CSHA512 SHAChecksum512;
			SHAChecksum512.init();
			SHAChecksum512.update(RandomData, 50);
			SHAChecksum512.final(HashDataH);

			SHAChecksum512.init();
			SHAChecksum512.update((RandomData + 50), 50);
			SHAChecksum512.final(HashDataL);

			BYTE KeyData[ARIA_MASTER_KEY_LEN] = { 0 };
			for (int i = 0; i < ARIA_MASTER_KEY_LEN; i++) {
				KeyData[i] = HashDataH[i] ^ HashDataL[i + 30];
			}
			memcpy(m_pMasterKey, KeyData, 32);
		}
	}
	_SafeDeleteArray(RandomData);
	_SafeDeleteArray(pData);

	return bRe;
}