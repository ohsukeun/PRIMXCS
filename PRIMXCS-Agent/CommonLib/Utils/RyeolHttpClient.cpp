...
LPCSTR CHttpToolA::szDefUsrAgent = "MCG HTTP Client Class" ;
...
LPCSTR CHttpToolA::szDefBoundary = "----MCG-FB3B405B7EAE495aB0C0295C54D4E096-" ;
LPCSTR CHttpToolA::szDefUploadContType = "multipart/form-data; boundary=" "----MCG-FB3B405B7EAE495aB0C0295C54D4E096-" ;
LPCSTR CHttpToolA::szNULL = "NULL" ;
LPCSTR CHttpToolA::szEmptyString = "" ;
LPCSTR CHttpToolA::szColonSlashSlash = "://" ;


LPCWSTR CHttpToolW::szDefUsrAgent = L"MCG HTTP Client Class" ;
...
LPCWSTR CHttpToolW::szDefBoundary = L"----MCG-FB3B405B7EAE495aB0C0295C54D4E096-" ;
LPCWSTR CHttpToolW::szDefUploadContType = L"multipart/form-data; boundary=" L"----MCG-FB3B405B7EAE495aB0C0295C54D4E096-" ;
	...
	throw (Exception &)
{
	...
	DWORD dwFlags2 = 0 ;
	dwFlags2 =	SECURITY_FLAG_IGNORE_UNKNOWN_CA			|
	...
	throw ()
{
	...
	::sprintf (szBoundary, "----REAL-%.08x%.04x%.04x%.02x%.02x%.02x%.02x%.02x%.02x%.02x%.02x-"
	...
	throw ()
{
	...
	::swprintf (szBoundary, L"----REAL-%.08x%.04x%.04x%.02x%.02x%.02x%.02x%.02x%.02x%.02x%.02x-"
		, guid.Data1, guid.Data2, guid.Data3, guid.Data4[0], guid.Data4[1]
		, guid.Data4[2], guid.Data4[3], guid.Data4[4], guid.Data4[5], guid.Data4[6], guid.Data4[7]);
	...
	throw (Exception &)
{
	...
	try {
		...
		MapValue	newValue = { (PCSZ) szValue, DWORD(dwFlag) } ;
	...
	throw (Exception &)
{
	...
	MapValue		newValue = { NULL, DWORD(dwFlag) } ;
	if ( szValue ) {
...
throw (Exception & )
{
	...
	try {
		cchProxyUserName = m_szProxyUserName == NULL ? 0 : HttpTool::StringLen (m_szProxyUserName) ;
		cchProxyPassword = m_szProxyPassword == NULL ? 0 : HttpTool::StringLen (m_szProxyPassword) ;
	} catch (::SafeIntException & e) {
		throw (e) ;
	}
	...
	throw (Exception &)
{
	...
	try {
		...
		if ( hInternet == NULL ) {
			...
			// 타인아웃 설정
			DWORD dTimeout = 1000 * m_nConnectTimeoutSec;
			InternetSetOption(hInternet, INTERNET_OPTION_CONNECT_TIMEOUT, (void*)&dTimeout, sizeof(dTimeout));
			InternetSetOption(hInternet, INTERNET_OPTION_SEND_TIMEOUT, (void*)&dTimeout, sizeof(dTimeout));
			InternetSetOption(hInternet, INTERNET_OPTION_RECEIVE_TIMEOUT, (void*)&dTimeout, sizeof(dTimeout));
			InternetSetOption(hInternet, INTERNET_OPTION_DATA_RECEIVE_TIMEOUT, (void*)&dTimeout, sizeof(dTimeout));
			InternetSetOption(hInternet, INTERNET_OPTION_DATA_SEND_TIMEOUT, (void*)&dTimeout, sizeof(dTimeout));
...
// 타인아웃 설정 함수
template <typename HttpTool, typename HttpEncoder>
void CHttpClientT<HttpTool, HttpEncoder>::SetConnectTimeout(UINT nTimeoutSec)
{
	m_nConnectTimeoutSec = nTimeoutSec;
}
