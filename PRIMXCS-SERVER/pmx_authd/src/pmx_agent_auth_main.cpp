/// @file pmx_agent_auth_main.cpp
/// @author Park, sung soo
/// @date 2023-07-25
/// PRIMX-CS agent authentication service main
...
#ifndef __PMX_AGENT_AUTH_MAIN_CPP__
#define __PMX_AGENT_AUTH_MAIN_CPP__
...
INT main(IN INT argc, PCHAR argv[])
{
	...
	if (FALSE == bDebug)
	{
		...
		_LOGGER_INIT_(BIN_STR, PMX_A_CONF_FILE_DIR, PMX_PATH_PRIMXAUTH_LOGS);
		_LOGGER_RELEASE;
	}
	// debug mode
	else
	...
	{
		WCString sCmd = "";
		sCmd.Format("echo %u > %s/%s.pid", getpid(), PMX_A_PID_FILE_DIR_PATH, BIN_STR);
	...
	// run main
	CAgentAuthSvr* pObj = new CAgentAuthSvr();
	if (FALSE == pObj->Init())
	{
...
//-----------------------------------------------------------------------------
#endif // __PMX_AGENT_AUTH_MAIN_CPP__
