...
#ifndef __PMX_AGENT_UPDATE_SVR_CPP__
#define __PMX_AGENT_UPDATE_SVR_CPP__
...
// METHOD DEFINITIONS
//-----------------------------------------------------------------------------
CAgentUpdateSvr::CAgentUpdateSvr()
{
	m_pAgentUpdateHd     = NULL;
	m_nConcurrentSession = SOCK_MAX_CLIENT_CNT;
	m_nWorkThreadCount   = 5;
	m_nAgentUpdatePort   = PMX_DEFAULT_UPDATE_PORT;
}
...
SWC_BOOL CAgentUpdateSvr::LoadConfig(OUT WCString& sAgentUpdateIP,
									 ...
									 OUT WCString& sDBName)
{
	...
	sAgentUpdateIP   = pConfMgr->GetNetworkUpdateSvrIP();
	nAgentUpdatePort = (USHORT)pConfMgr->GetNetworkUpdateSvrPort();
...
SWC_BOOL CAgentUpdateSvr::Init()
{
	...
	// load config.
	WCString sAgentUpdateIP = "", sDBAddr = "", sDBUser = "", sDBPwd = "", sDBName = "";
	USHORT   nAgentUpdatePort = 0, nDBPort = 0;

	if (FALSE == LoadConfig(sAgentUpdateIP, nAgentUpdatePort, sDBAddr, nDBPort, sDBUser, sDBPwd, sDBName))
	{
		return FALSE;
	}

	// init. agent update channel handler ------------------------------------
	m_pAgentUpdateHd = new CSSLHandler();

	sAgentUpdateIP.MakeUpper(); sAgentUpdateIP.Trim();
	m_nAgentUpdatePort = nAgentUpdatePort;
	...
	// agent update session handler init.
	if (FALSE == m_pAgentUpdateHd->Init((sAgentUpdateIP == "ANY") ? NULL : LPCSTR(sAgentUpdateIP), 
										 m_nAgentUpdatePort, 
	...
	{
		_LOG_ERR("agent update session handler init. failed");
		return FALSE;
	}

	m_pAgentUpdateHd->m_sModuleName = "primx-cs agent update session handler";
	...
	m_pAgentUpdateHd->SetProperties(&attr);

	// init. work threads
	INT nID = 0;
	for (nID = 0; nID < m_pAgentUpdateHd->GetWorkThCount(); nID++)
	{
		CAgentUpdateIO* pNode = new CAgentUpdateIO();
		...
		{
			m_pAgentUpdateHd->m_pWorkTh[nID] = pNode;
			m_pAgentUpdateHd->m_pWorkTh[nID]->Init(m_pAgentUpdateHd);
		}
	}
	//-------------------------------------------------------------------------
...
SWC_BOOL CAgentUpdateSvr::Run()
{
	_LOG_DBG("enter ...");

	if (FALSE == m_pAgentUpdateHd->Run())
	{
		_LOG_ERR("%s running failed", LPCSTR(m_pAgentUpdateHd->m_sModuleName));
		return FALSE;
	}
	m_pAgentUpdateHd->SetEnableClientAccept();

	_LOG_MSG("%s started", LPCSTR(m_pAgentUpdateHd->m_sModuleName));

	return TRUE;
}

VOID CAgentUpdateSvr::Exit()
{
	if (NULL != m_pAgentUpdateHd)
	{
		delete m_pAgentUpdateHd;
		m_pAgentUpdateHd = NULL;
	}
	WF_SSL_exit();
}

VOID CAgentUpdateSvr::DelCoreFiles(IN const CHAR* sDir, IN const CHAR* sFind, IN UINT nRemain)
{
	DIR*           pDir = NULL;
...
VOID CAgentUpdateSvr::CalcWorkThreadCount()
{
...
//-----------------------------------------------------------------------------
#endif // __PMX_AGENT_UPDATE_SVR_CPP__
