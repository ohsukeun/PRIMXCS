...
WCString CPmxXmlIO::MakeKeepAliveReplyXml(IN SWC_BOOL bSuccess, 
										  ...
										  IN WCString zTryMenuEnable,
                                                                                  IN WCString zOpenArea,
                                                                                  IN WCString zPersonalAccess,
                                                                                  IN WCString zOptionDisplay,
                                                                                  IN WCString zSafetyZone,
                                                                                  IN WCString zAreaClose,
                                                                                  IN WCString zEnc,
	 									  IN WCString sPolicyUpdateQuery,
										  IN WCString sPolicyFileName,
										  IN WCString sOutLastPolicyDate,
										  IN WCString sOutLastPolicyID,
										  IN WCString tZoneMap,
										  ...
										  IN WCString sFailMsg)
{
	...
	if (sExpandCommmand == PMX_XML_VAL_NONE)
	{
		...
		xml.AddElem("PolicyFileName",      LPCSTR(sPolicyFileName));
		xml.AddElem("LastPolicyDate",      LPCSTR(sOutLastPolicyDate));
		xml.AddElem("LastPolicyID",      LPCSTR(sOutLastPolicyID));
	...
	if (sExpandCommmand == PMX_XML_VAL_NONE)
	{
		...
		xml.AddElem(PMX_XML_ELM_ZTRYMENUENABLE, LPCSTR(zTryMenuEnable));
		xml.AddElem(PMX_XML_ELM_ZOPENAREA, LPCSTR(zOpenArea));
		xml.AddElem(PMX_XML_ELM_ZPERSONALACCESS, LPCSTR(zPersonalAccess));
		xml.AddElem(PMX_XML_ELM_ZOPTIONDISPLAY, LPCSTR(zOptionDisplay));
		xml.AddElem(PMX_XML_ELM_ZSAFETYZONE, LPCSTR(zSafetyZone));
		xml.AddElem(PMX_XML_ELM_ZAREACLOSE, LPCSTR(zAreaClose));
		xml.AddElem(PMX_XML_ELM_ZENC, LPCSTR(zEnc));
		xml.AddElem(PMX_XML_ELM_TZONEMAP, LPCSTR(tZoneMap));
		// </Menu>
		xml.OutOfElem();
...
SWC_BOOL CPmxXmlIO::ExtracrtUpdateFileRequest(OUT INT& nProductType, OUT WCString& sInstalledVersion, OUT WCString& sTypeFull)
{
	MakeLastError(m_Markup, PMX_ERR_XML_RW_SUCCESS);
	...
	else
	{
		...
		else
		{
			...
			{
				_LOG_ERR("unkonwn or invalid product type:%s", LPCSTR(sTypenew));
				sTypeFull = LPCSTR(sTypenew);
				return FALSE;
			}
		}
		// <UpdateVersion>
...
// pmx_io.cpp
WCString CPmxXmlIO::GetUpdateType() {
	return GetElmValueStrByPath("DT:UpdateFileRequest/Type");
}

WCString CPmxXmlIO::GetPolicyFileName() {
    WCString sType = GetUpdateType();  // ex) POLICY|POL-2025021700001.xml
    int pos = sType.Find('|');
    if (pos > 0) {
        return sType.Mid(pos + 1);
    }
    return "";
}
