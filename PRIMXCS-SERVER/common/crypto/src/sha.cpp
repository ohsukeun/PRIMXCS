...
CSHA::CSHA(int iMethod)
{
	...
	switch(iMethod)
	{
		...
		default:
		{
			string ostrMsg = "FileDigest ERROR: in CSHA() Constructor, Illegal Method " + to_string(iMethod) + "!";
...
WCString CSHA::GetSHAData(IN const WCString& sFilePath, OUT WCString& sErrorMsg)
{
	WCString sRet = (CHAR*)"";

	if (0 == sFilePath.GetLength()) { sErrorMsg = "empty file path"; return sRet; }

	filesystem::path filePath(LPCSTR(sFilePath));
	if (false == filesystem::exists(filePath)) { sErrorMsg = "file not found"; return sRet; }
	if (false == filesystem::is_directory(filePath)) { sErrorMsg = "path is a directory"; return sRet; }

	uintmax_t lFileSize = filesystem::file_size(filePath);
	if (0 == lFileSize) { sErrorMsg = "file size is zero"; return sRet; }

	unsigned char md[PRIMX_SHA_DIGEST_LENGTH+2] = { 0 };
	PBYTE pBuffer = NULL;
	try
	{
		ifstream hFile(LPCSTR(sFilePath), ios::binary);
		int nLength = 0;

		hFile.exceptions ( ifstream::badbit );

		Reset();
		if (true == hFile.is_open())
		{
			hFile.seekg(0, hFile.end);
			nLength = (int)hFile.tellg();
			hFile.seekg(0, hFile.beg);
			
			pBuffer = new BYTE[nLength];
			memset(pBuffer, 0x00, nLength);

			hFile.read((PCHAR)pBuffer, nLength);
			AddData((CHAR*)pBuffer, nLength);
			hFile.close();
	...
	if (SHA160 == m_iMethod) nSize = 160/8;
	if (SHA256 == m_iMethod) nSize = 256/8;
	if (SHA384 == m_iMethod) nSize = 384/8;
	if (SHA512 == m_iMethod) nSize = 512/8;

	CBase64Ex Base64;
	int nBase64Len = Base64.EncodeGetRequiredLength(nSize);
	if (0 < nBase64Len)
	{
		CHAR* pBase64md = new CHAR[nBase64Len + 1];

		memset(pBase64md, 0x00, nBase64Len+1);
		if (TRUE == Base64.Encode((unsigned char*)md, nSize, pBase64md, &nBase64Len)) 
		{
			sRet = pBase64md;
		}
		delete [] pBase64md;
