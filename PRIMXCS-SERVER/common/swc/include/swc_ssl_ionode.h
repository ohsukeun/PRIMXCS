/*****************************************************************************
 *                                                                           *
 * File Name : swc_ssl_ionode.h                                              *
 * Theme     : ssl recv / write object definitions                           *
 * Author    : Park, sung soo                                                *
 * History   : 1. created by  Park, sung soo '10. 04. 13                     *
 *                                                                           *
 *****************************************************************************/

/// @file swc_ssl_ionode.h
/// @author Park, sung soo
/// @date 2010-04-13
/// @brief ssl recv / write object definitions
...
#ifndef __SWC_SSL_IONODE_H__
#define __SWC_SSL_IONODE_H__
...
public :
	// socket I/O related
	VOID        Close              (IN SWC_BOOL bClose = FALSE);
	VOID        Reset              ();
	VOID        ResetRecv          ();
	VOID        ResetSend          ();
	...
	VOID        SetTempBufAlloc    (IN INT nlen);
	const CHAR* GetTempBuf         () const  { return m_pTempBuf; };
	VOID        InitAsRecv         (IN INT nlen);
	...
	VOID        UpdateTimeLastIO   () { time_t tCur = 0; time  (&tCur); m_tLastIO = tCur; };
	...
	// SSL related
	VOID        SetSSL             (IN SSL* pssl);
	SSL*        GetSSL             ();
	INT         Write              (IN PVOID buf, IN INT len);
	INT         Write              (IN PVOID buf, IN INT len, IN INT timeout, IN SWC_BOOL bFlexTimeOut = FALSE);
	INT         Read               (OUT PVOID buf, IN INT len);
	INT         Read               (OUT PVOID buf, IN INT len, IN INT timeout, IN SWC_BOOL bFlexTimeOut = FALSE);
	INT         GetSSLError        (IN SWC_BOOL bout);
	...
	INT         GetSSLError        (IN SOCKET nsfd, IN INT nresult);
