...
function genTree(data) {
    ...
    $('#tree').jstree({
        ...
        "types": {
            "default": {
                "icon": "fa fa-folder text-primary" // 기본 폴더 아이콘
            },
            "file": {
                "icon": "fa fa-file text-primary" // 파일 아이콘
            },
            "policyDefinitions": {
                "icon": "fa fa-cogs text-warning" // policyDefinitions 항목 아이콘
            },
            "categories": {
                "icon": "fa fa-list text-success" // categories 항목 아이콘
            },
            "custom": {
                "icon": "url('/path/to/custom/icon.png')" // 이미지 아이콘 사용 예시
            }
        },
        "plugins": ["types"]
    });
...
function displayPolicyDetails(policyId) {
    ...
    if (policy) {
        var policyDetailsHtml = `
            ...
            <div class="policy-detail"><strong>설명:</strong> ${policy.explainText || '설명 없음'}</div>
            <div class="policy-settings">
                <label>
                    <input type="radio" name="${policyId}" value="not_configured" /> 구성되지 않음
                </label>
                <label>
                    <input type="radio" name="${policyId}" value="enabled" /> 사용
                </label>
                <label>
                    <input type="radio" name="${policyId}" value="disabled" /> 사용 안 함
                </label>
            </div>
        `;
        $('#policy-info').html(policyDetailsHtml);
    }
}
</script>
