...
CREATE TABLE IF NOT EXISTS `COMTNEMPLYRINFO` (
  `EMPLYR_SEQ` bigint(20) NOT NULL AUTO_INCREMENT COMMENT 'INDEX 순번',
  ...
  `PHONE_NO` varchar(20) DEFAULT NULL COMMENT '이동전화번호',
  `EMAIL_ADRES` varchar(50) DEFAULT NULL COMMENT '이메일주소',
  `OFCPS_NM` varchar(60) DEFAULT NULL COMMENT '직위명',
  `USE_YN` char(1) DEFAULT 'Y' COMMENT '정보 사용 여부 (Y: 사용, N:미사용)',
  `USER_TYPE` tinyint(4) DEFAULT 0 COMMENT '사용자 타입. (0: 일반사용자(Default), 1: 게스트 사용자)',
  `USER_ROLE` varchar(255) DEFAULT NULL COMMENT '권한. ADMIN/User',
  `EMPLYR_STTUS_CODE` varchar(1) DEFAULT 'A' COMMENT '사용자상태코드(A, D, P)',
  `ESNTL_ID` char(20) DEFAULT NULL COMMENT '고유ID',
  `CRTFC_DN_VALUE` varchar(100) DEFAULT NULL COMMENT '인증DN값',
  `INSERTED_BY_HR_YN` char(1) DEFAULT 'N' COMMENT '인사연동으로 생성됨 여부 (‘Y’: 인사연동됨, ‘N’: 관리자 생성(Default))',
  `PWD_CHANGED` char(1) NOT NULL DEFAULT 'N' COMMENT '패스워드 변경 여부. (‘Y’: 변경, ‘N’: 미변경(Default))',
  `IS_PERIOD` char(1) DEFAULT 'N' COMMENT '사용 기간을 지정했는지 여부. (‘Y’: 지정, ‘N’: 미지정(Default))',
  `INSERT_DATE` datetime DEFAULT NULL COMMENT '생성 일자',
  `EXPIRED_DATE` datetime DEFAULT NULL COMMENT '유효 기간',
  `PWD_EXPIRED_DATE` datetime DEFAULT NULL COMMENT '패스워드 만료일',
  ...
  `CHG_PWD_LAST_PNTTM` datetime DEFAULT NULL COMMENT '비밀번호 변경 최종 일시',
  PRIMARY KEY (`EMPLYR_SEQ`,`EMPLYR_ID`),
  UNIQUE KEY `COMTNEMPLYRINFO_PK` (`EMPLYR_SEQ`,`EMPLYR_ID`),
  UNIQUE KEY `uidx_t_user_master` (`COMPANY_ID`,`EMPLYR_ID`),
  UNIQUE KEY `uidx_c_email_addr` (`EMAIL_ADRES`)
) ENGINE=InnoDB AUTO_INCREMENT=1156 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='업무사용자정보';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='권한정보';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='롤정보';
...
CREATE TABLE IF NOT EXISTS `COMTNDEPTINFOMGR` (
  `DEPT_SEQ` int(11) NOT NULL AUTO_INCREMENT COMMENT '부서 ID',
  `COMPANY_ID` varchar(20) NOT NULL COMMENT '회사 ID',
  `DEPT_ID` varchar(50) NOT NULL COMMENT '부서 ID',
  `DEPT_NAME` varchar(20) NOT NULL COMMENT '부서 이름',
  `INSERTED_BY_HR_YN` char(1) DEFAULT 'N' COMMENT '인사연동으로 생성됨 여부 (‘Y’: 인사연동됨, ‘N’: 관리자가 생성(Default))',
  `PARENT_ID` varchar(20) DEFAULT NULL COMMENT '부모 부서 ID  상위가 없으면 NULL',
  `LEVEL` int(11) NOT NULL COMMENT '부서 계층 레벨',
  `SORT_ORDER` int(11) DEFAULT 0 COMMENT '정렬 순서',
  `CREATED_DATE` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '생성 시간',
  `UPDATE_DATE` datetime DEFAULT NULL COMMENT '수정 일자',
  `USE_YN` varchar(2) DEFAULT 'Y' COMMENT '부서 사용 유무',
...
CREATE TABLE `COMTNEMMGRINFO` (
  `MGR_NO` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '번호',
  `MANAGER_ID` varchar(50) NOT NULL COMMENT '관리자 ID',
  `ALLOWED_IP` varchar(15) DEFAULT NULL COMMENT '허용 IP 주소',
  `INSERT_DATE` datetime DEFAULT NULL COMMENT '생성 일자',
  `UPDATE_DATE` datetime DEFAULT NULL COMMENT '수정 일자',
  `ACCOUNT_EXPIRY_DATE` datetime DEFAULT NULL COMMENT '계정 유효 일자',
  PRIMARY KEY (`MGR_NO`),
  UNIQUE KEY `uidx_t_manager_allowed_ip` (`MANAGER_ID`,`ALLOWED_IP`),
  KEY `idx_t_manager_allowed_ip` (`MANAGER_ID`,`ALLOWED_IP`)
) ENGINE=InnoDB AUTO_INCREMENT=130 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='관리자 접속 허용 IP 리스트';
...
CREATE TABLE IF NOT EXISTS COMTNROUGHMAP (
...
CREATE TABLE IF NOT EXISTS COMTNPRIVACYLOG
(
	REQUST_ID            VARCHAR(20) NOT NULL,
...
;
 ...
 CREATE TABLE IF NOT EXISTS COMTNCOMMUTE (
...
CREATE TABLE IF NOT EXISTS COMTNBLOG
(
...
CREATE TABLE IF NOT EXISTS COMTNBLOGUSER
(
	BLOG_ID             CHAR(20) NOT NULL,
	EMPLYR_ID             VARCHAR(20) NOT NULL,
	MNGR_AT               CHAR(1) NOT NULL,
...
CREATE TABLE IF NOT EXISTS COMTCMOBILEIDSERVICE
(
...
CREATE TABLE IF NOT EXISTS COMTSMOBILEIDTRXINFO
(
